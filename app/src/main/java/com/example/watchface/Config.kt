...
object Config {
    ...
    const val BRIGHTNESS_DIM_NIGHT = 0.08f
    const val BRIGHTNESS_DIM_ROOM = 0.20f
    const val BRIGHTNESS_DIM_SUN = 0.45f

    // Dual-stage Dim overlay alpha (gentle contrast so images remain clearly visible and bright)
    const val OVERLAY_ALPHA_NIGHT = 0.08f
    const val OVERLAY_ALPHA_ROOM = 0.00f
    ...
    // Image dimming factor in DIM mode (via ColorMatrix, keeping high clarity, brightness and rich colors)
    const val IMAGE_DIM_SCALE = 0.85f

    // Anti-burn-in multi-phase pixel shift algorithm
    const val BURN_IN_RADIUS_DP = 5.5f
    const val BURN_IN_CYCLE_MINUTES = 120

    // Time-based Circadian Diurnal Brightness curve parameters (OLED protection with comfortable brightness)
    const val CIRCADIAN_NIGHT_FACTOR = 0.70f // 23:00 - 05:30 night comfortable brightness
    const val CIRCADIAN_DAWN_FACTOR = 0.90f  // 05:30 - 08:30 morning transition
    const val CIRCADIAN_DAY_FACTOR = 1.00f   // 08:30 - 17:30 peak day
    const val CIRCADIAN_DUSK_FACTOR = 0.85f  // 17:30 - 21:00 evening transition
    const val CIRCADIAN_EVENING_FACTOR = 0.75f // 21:00 - 23:00 pre-sleep dimmed
